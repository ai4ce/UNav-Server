...
from unav.localizer.tools.io import (
    load_colmap_model,
...
class UNavLocalizer:
    ...
    def _init_models(self):
        ...
        self.local_matcher = (
            Local_extractor(feat_cfg["local_extractor_config"])
            .matcher()
            .to(self.device)
        )
            ...
            for building, floors in bld_dict.items():
                for floor in floors:
                    ...
                    feature_dir = os.path.join(
                        self.config.data_final_root, place, building, floor, "features"
                    )
                    self.global_feat_paths[key] = os.path.join(
                        feature_dir,
                        f"global_features_{self.config.global_descriptor_model}.h5",
                    )
                    self.local_feat_paths[key] = os.path.join(
                        feature_dir, "local_features.h5"
                    )
                    model_dir = os.path.join(
                        self.config.data_final_root,
                        place,
                        building,
                        floor,
                        "colmap_map",
                    ...
                    try:
                        ...
                        print(
                            f"[✓] Loaded COLMAP model for {key}: {len(frames_by_name)} frames"
                        )
                    ...
                    if os.path.exists(h5_path):
                        try:
                            ...
                            print(
                                f"[✓] Loaded global features for {key}: {len(names)} images"
                            )
                        except Exception as e:
                            print(
                                f"[WARNING] Could not load global features for {key}: {e}"
                            )
                    ...
                    if os.path.exists(transform_path):
                        try:
                            ...
                            print(
                                f"[✓] Loaded transform matrix for {key}: shape={matrix.shape}"
                            )
                        except Exception as e:
                            print(
                                f"[WARNING] Could not load transform matrix for {key}: {e}"
                            )
    ...
    def vpr_retrieve(self, global_feat, top_k=None):
        ...
        print(f"🔍 [VPR DEBUG] vpr_retrieve called with topk={topk}")
        results = search_vpr_topk_candidates(
        ...
        for i, item in enumerate(results):
            if isinstance(item, (list, tuple)) and len(item) >= 3:
                print(
                    f"   candidate[{i}]: map_key={item[0]}, img={item[1]}, score={item[2]:.4f}"
                )
            else:
    ...
    def batch_local_matching_and_ransac(self, local_feat_dict, candidates_data):
        ...
        if isinstance(local_feat_dict, dict):
            print(f"   local_feat_dict keys: {list(local_feat_dict.keys())[:10]}")
            for k, v in local_feat_dict.items():
                if hasattr(v, "shape"):
                    print(
                        f"   local_feat_dict[{k}]: shape={v.shape}, dtype={getattr(v, 'dtype', 'N/A')}"
                    )
                elif isinstance(v, (list, dict)):
                    print(f"   local_feat_dict[{k}]: {type(v).__name__}, len={len(v)}")
        if isinstance(candidates_data, dict):
            print(f"   candidates_data: {len(candidates_data)} candidates")
            for i, (k, v) in enumerate(list(candidates_data.items())[:5]):
                if isinstance(v, dict):
                    print(f"   candidate[{k}]: keys={list(v.keys())[:8]}")
                else:
                    print(f"   candidate[{k}]: type={type(v).__name__}")

        result = batch_local_matching_and_ransac(
            local_feat_dict,
            candidates_data,
            matcher=self.local_matcher,
            feature_score_threshold=self.config.localization_config.get(
        ...
        print(f"🔍 [MATCH DEBUG] batch_local_matching_and_ransac results:")
        print(f"   best_map_key: {best_map_key}")
        ...
        if results:
            ...
            print(
                f"   No results - all candidates were filtered out by RANSAC/matching"
            )
    ...
    def localize(
        self, query_img: np.ndarray, refinement_queue: dict, top_k: int = None, **kwargs
    ) -> Dict[str, Any]:
        """
        ...
        map_queue = refinement_queue.get(
            best_map_key, {"pairs": [], "initial_poses": [], "pps": []}
        )
        try:
