...
| 5 | `d240c92` | Add `pp=None` to the `traced_match` wrapper in `localizer.py:137` and forward it. Reason: the upstream `UNavLocalizer.batch_local_matching_and_ransac` (in `unav/`) now accepts a `pp` kwarg per the `aa60dc9` submodule update; the wrapper had a fixed signature without `pp`, so calls like `orig_match(self, ..., pp=pp)` raised `TypeError: ... got an unexpected keyword argument 'pp'`. | **Not yet observed in the running container.** The current logs still show the same `pp` error because the deployment is using a cached image layer that pre-dates `d240c92`. | **Action needed: redeploy Modal** so the new `localizer.py` is picked up. Once redeployed, the MASt3R pipeline will reach the matcher and either produce a pose or a meaningful "no inliers" failure. |
